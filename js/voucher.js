...
function addVoucher() {
  const user = getCurrentUser();
  if (!isAdmin(user)) {
    customAlert('Bạn không có quyền thêm voucher!');
    return;
  }
  ...
  push(vouchersRef, newVoucher).then(() => {
    customAlert('Thêm voucher thành công!');
    closeAddVoucherModal();
  }).catch((error) => {
    customAlert('Lỗi khi thêm voucher: ' + error.message);
  });
}
...
function editVoucher(index) {
  const user = getCurrentUser();
  if (!isAdmin(user)) {
    customAlert('Bạn không có quyền sửa voucher!');
    return;
  }
...
function updateVoucher() {
  ...
  update(voucherRef, updatedVoucher).then(() => {
    customAlert('Cập nhật voucher thành công!');
    closeEditVoucherModal();
  }).catch((error) => {
    customAlert('Lỗi khi cập nhật voucher: ' + error.message);
  });
}
...
function deleteVoucher(index) {
  const user = getCurrentUser();
  if (!isAdmin(user)) {
    customAlert('Bạn không có quyền xóa voucher!');
    return;
  }
    ...
    if (data) {
      ...
      if (voucherKeys[index]) {
        ...
        remove(voucherRef).then(() => {
          customAlert('Xóa voucher thành công!');
        }).catch((error) => {
          customAlert('Lỗi khi xóa voucher: ' + error.message);
        });
      }
    }
  });
...
// Hàm hiển thị thông báo lỗi, thành công bằng popup tùy chỉnh
