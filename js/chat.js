...
window.sendMessage = async function() {
    ...
    if (chatSnap.exists()) {
        ...
        if (isBlocked) {
            customAlert('Bạn đã chặn người này. Không thể gửi tin nhắn.');
            return;
        }
        
        if (isBlockedBy) {
            customAlert('Bạn đã bị chặn bởi người này. Không thể gửi tin nhắn.');
            return;
        }
    }
    ...
    if (confirm('Bạn có chắc chắn muốn xóa đoạn chat này? Hành động này không thể hoàn tác.')) {
        ...
        set(messagesRef, null).then(() => {
            set(chatRef, null).then(() => {
                customAlert('Đã xóa đoạn chat thành công!');
                currentChatId = null;
                ...
                customAlert('Có lỗi xảy ra khi xóa đoạn chat!');
            });
        }).catch(error => {
            console.error('Lỗi khi xóa messages:', error);
            customAlert('Có lỗi xảy ra khi xóa tin nhắn!');
        });
    }
};
                ...
                if (alreadyBlocked) {
                    ...
                    if (confirm(`Bạn có chắc chắn muốn chặn ${otherUserName || 'người này'}?\n\nSau khi chặn:\n• Không thể nhận/gửi tin nhắn\n• Chat sẽ bị xóa\n• Không thể tạo chat mới\n\nBạn có thể bỏ chặn sau trong "Quản lý danh sách chặn"`)) {
                        ...
                        set(blockedRef, {
                            ...
                            customAlert('Có lỗi xảy ra khi chặn người dùng!');
                        });
                    }
                }
            });
    ...
    get(blockedRef).then(snap => {
        if (snap.exists()) {
            ...
            set(blockedRef, null).then(() => {
                customAlert(`Đã bỏ chặn ${blockedUser.userName || 'người dùng'} thành công!\n\nBây giờ bạn có thể:\n• Nhận tin nhắn từ họ\n• Gửi tin nhắn cho họ\n• Tạo chat mới với họ`);
                ...
                customAlert('Có lỗi xảy ra khi bỏ chặn!');
            });
        }
    });
};
...
window.unblockAllUsers = function() {
    if (confirm('Bạn có chắc chắn muốn bỏ chặn tất cả người dùng?\n\nHành động này không thể hoàn tác.')) {
        ...
        set(blockedRef, null).then(() => {
            customAlert('Đã bỏ chặn tất cả người dùng thành công!');
            loadBlockedUsers();
        }).catch(error => {
            console.error('Lỗi khi bỏ chặn tất cả:', error);
            customAlert('Có lỗi xảy ra khi bỏ chặn tất cả!');
        });
    }
};
        ...
        if (snap.exists()) {
            ...
            if (reportReason && reportReason.trim()) {
                ...
                set(newReportRef, {
                    ...
                    customAlert('Báo cáo đã được gửi thành công! Chúng tôi sẽ xem xét trong thời gian sớm nhất.');
                    document.getElementById('roomMenuDropdown').classList.remove('show');
                }).catch(error => {
                    console.error('Lỗi khi gửi báo cáo:', error);
                    customAlert('Có lỗi xảy ra khi gửi báo cáo!');
                });
            }
        }
    });
