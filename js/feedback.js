...
document.getElementById('feedbackForm').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (!name || !email || !content) {
            customAlert('Vui lòng điền đầy đủ thông tin trước khi gửi cho chúng tôi!');
            return;
        }
        
        if (content.length < 10) {
            customAlert('Nội dung góp ý phải có ít nhất 10 ký tự!');
            return;
        }
        
        if (content.length > 1000) {
            customAlert('Nội dung góp ý không được vượt quá 1000 ký tự!');
            return;
        }
        ...
        if (!emailRegex.test(email)) {
            customAlert('Email không hợp lệ!');
            return;
        }
        ...
        customAlert('Có lỗi xảy ra khi gửi phản hồi. Vui lòng thử lại sau!');
    } finally {
